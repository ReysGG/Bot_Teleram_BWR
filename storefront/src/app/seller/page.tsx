...
import { SellerStatGrid } from "@/components/seller/seller-primitives";
...
export default async function SellerPage() {
  ...
  if (access.kind !== "active" || !access.seller) return <section className="seller-panel"><h1>Portal seller</h1><p>{access.kind === "disabled" ? "Portal seller belum diaktifkan di lingkungan ini." : access.kind === "guest" ? "Masuk dengan akun yang menerima undangan seller." : "Akses seller memerlukan undangan dan akun yang aktif."}</p>{access.kind === "guest" ? <Link className="button button-primary" href="/seller/login">Masuk sebagai seller</Link> : <Link className="button button-quiet" href="/shop">Kembali ke toko</Link>}</section>;
  ...
  return <>
    ...
    <SellerStatGrid items={[{label:"Produk aktif/nonaktif",value:String(dashboard.productCount),href:"/seller/products"},{label:"Penjualan tercatat",value:String(dashboard.saleCount),href:"/seller/sales"},{label:"Saldo tersedia",value:money(wallet?.available),href:"/seller/balance"},{label:"Saldo ditahan",value:money(wallet?.held),href:"/seller/balance"}]}/>
