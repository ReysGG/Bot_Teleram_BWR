...
export async function POST(request: NextRequest) {
  try {
    ...
    await setLegacyQrisFallbackEnabled({ enabled: false, actor: `admin:${admin.email}` });
    return NextResponse.redirect(appRoute("/admin/payment-settings/qris?notice=legacy_qris_disabled"), 303);
  } catch (error) {
