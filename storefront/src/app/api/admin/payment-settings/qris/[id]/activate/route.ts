...
import { activateQrisMerchant } from "@/server/payment/qris-merchant-service";
...
export async function POST(
  ...
  try {
    ...
    await activateQrisMerchant({ id, actor: `admin:${admin.email}` });
    return NextResponse.redirect(appRoute(`${returnTo}?notice=qris_activated`), 303);
  } catch (error) {
