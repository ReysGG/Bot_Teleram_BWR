...
archive=base/'storefront-flow-image.tar.gz'
manifest=json.loads((base/'storefront-flow-image.json').read_text())
...
image=json.loads(subprocess.check_output(['docker','image','inspect','telegram-app:storefront-flow-20260916']))[0]
assert image['Architecture']=='amd64' and image['Os']=='linux'
subprocess.run(['python3',str(base/'verify-storefront-flow-content.py')],check=True)
