...
archive=base/'checkout-polish-image.tar.gz'
manifest=json.loads((base/'checkout-polish-image.json').read_text())
...
image=json.loads(subprocess.check_output(['docker','image','inspect','telegram-app:checkout-polish-20260916']))[0]
assert image['Architecture']=='amd64' and image['Os']=='linux'
subprocess.run(['python3',str(base/'verify-checkout-polish-content.py')],check=True)
