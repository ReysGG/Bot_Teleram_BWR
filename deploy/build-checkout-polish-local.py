...
build=Path.home()/'.codex'/'build-cache'/'telegram-checkout-polish'
build.mkdir(parents=True,exist_ok=True)
archive=root/'deploy/checkout-polish-source.tar.gz'
...
(root/'deploy/checkout-polish-release-manifest.json').write_text(json.dumps({'sha256':hashlib.file_digest(archive.open('rb'),'sha256').hexdigest()}))
...
subprocess.run(['docker','build','--platform','linux/amd64','--target','runner','-t','telegram-app:checkout-polish-20260916',str(build)],check=True)
