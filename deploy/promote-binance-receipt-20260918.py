...
candidate=('telegram-app' if kind=='backend' else 'telegram-storefront')+':binance-receipt-20260918'
...
backup=base/('backups/binance-receipt-20260918-'+str(int(time.time())));backup.mkdir(mode=0o700,exist_ok=False)
...
subprocess.run(['docker','tag',old,alias.split(':')[0]+':rollback-binance-receipt-20260918'],check=True)
db=None
if kind=='backend':
