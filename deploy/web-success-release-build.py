...
if '--background' in sys.argv:
    ...
    print('Isolated web-success tests and backend build started'); sys.exit(0)
network = 'web-success-review-isolated-20260916'
database = 'web-success-review-db-20260916'
created_net = False
created_db = False
try:
    state.write_text(json.dumps({'status': 'testing'}))
    source = base / 'source'; source.mkdir(exist_ok=False)
    archive = Path('/home/ubuntu/web-success-release-source.tar.gz')
    manifest = json.loads(Path('/home/ubuntu/web-success-release-manifest.json').read_text())
    ...
    run(['docker', 'build', '--target', 'deps', '-t', 'telegram-web-success-deps:20260916', str(source)])
    (source / 'Dockerfile.web-success-test').write_text('FROM telegram-web-success-deps:20260916\nCOPY . .\nRUN npx --no-install prisma generate\nCMD ["sh", "-c", "npx --no-install prisma migrate deploy && npx --no-install vitest run tests/storefront-web-order-db.test.ts tests/storefront-product-guidance.test.ts tests/web-success-announcement.test.ts tests/web-success-worker.test.ts tests/success-channel.test.ts"]\n')
    run(['docker', 'build', '-f', str(source / 'Dockerfile.web-success-test'), '-t', 'telegram-web-success-tests:20260916', str(source)])
    run(['docker', 'network', 'create', '--internal', network], stdout=subprocess.DEVNULL); created_net = True
    run(['docker', 'run', '-d', '--name', database, '--network', network, '--memory', '512m', '--tmpfs', '/var/lib/postgresql/data:rw,size=256m', '-e', 'POSTGRES_USER=web-successtest', '-e', 'POSTGRES_PASSWORD=disposable-only', '-e', 'POSTGRES_DB=web-success_disposable', 'postgres:17-alpine'], stdout=subprocess.DEVNULL); created_db = True
    for attempt in range(30):
        if subprocess.run(['docker', 'exec', database, 'pg_isready', '-U', 'web-successtest'], capture_output=True).returncode == 0: break
        time.sleep(1)
    else: raise RuntimeError('Disposable DB not ready')
    url = 'postgresql://web-successtest:disposable-only@' + database + ':5432/web-success_disposable'
    ...
    run(args + ['telegram-web-success-tests:20260916'], env=env)
    ...
    with (source / '.dockerignore').open('a') as f: f.write('\ntests/\nDockerfile.web-success-test\n')
    run(['docker', 'build', '--target', 'runner', '-t', 'telegram-app:web-success-20260916', str(source)])
    out = base / 'backend-image.tar.gz'
    with gzip.open(out, 'wb', compresslevel=1) as output:
        proc = subprocess.Popen(['docker', 'save', 'telegram-app:web-success-20260916'], stdout=subprocess.PIPE)
