...
archive=base/'binance-receipt-image.tar.gz'
manifest=json.loads((base/'binance-receipt-image.json').read_text())
...
image=json.loads(subprocess.check_output(['docker','image','inspect','telegram-app:binance-receipt-20260918']))[0]
assert image['Architecture']=='amd64' and image['Os']=='linux'
subprocess.run(['python3',str(base/'verify-binance-receipt-content.py')],check=True)
