...
candidate=('telegram-app' if kind=='backend' else 'telegram-storefront')+':message-layout-20260916'
...
backup=base/('backups/message-layout-20260916-'+str(int(time.time())));backup.mkdir(mode=0o700,exist_ok=False)
...
subprocess.run(['docker','tag',old,alias.split(':')[0]+':rollback-message-layout-20260916'],check=True)
db=None
if kind=='backend':
