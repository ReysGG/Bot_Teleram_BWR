...
candidate=('telegram-app' if kind=='backend' else 'telegram-storefront')+':web-fix-20260916'
...
backup=base/('backups/web-fix-20260916-'+str(int(time.time())));backup.mkdir(mode=0o700,exist_ok=False)
...
subprocess.run(['docker','tag',old,alias.split(':')[0]+':rollback-web-fix-20260916'],check=True)
db=None
if kind=='backend':
