...
out=Path('deploy/checkout-polish-image.tar.gz')
with gzip.open(out,'wb',compresslevel=1) as f:
 p=subprocess.Popen(['docker','save','telegram-app:checkout-polish-20260916'],stdout=subprocess.PIPE)
...
Path('deploy/checkout-polish-image.json').write_text(json.dumps(manifest))
