...
build=Path.home()/'.codex'/'build-cache'/'telegram-binance-receipt'
build.mkdir(parents=True,exist_ok=True)
archive=root/'deploy/binance-receipt-source.tar.gz'
...
(root/'deploy/binance-receipt-release-manifest.json').write_text(json.dumps({'sha256':hashlib.file_digest(archive.open('rb'),'sha256').hexdigest()}))
...
subprocess.run(['docker','build','--platform','linux/amd64','--target','runner','-t','telegram-app:binance-receipt-20260918',str(build)],check=True)
