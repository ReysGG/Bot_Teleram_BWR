...
build=Path.home()/'.codex'/'build-cache'/'telegram-multi-invoice'
build.mkdir(parents=True,exist_ok=True)
archive=root/'deploy/multi-invoice-source.tar.gz'
...
(root/'deploy/multi-invoice-release-manifest.json').write_text(json.dumps({'sha256':hashlib.file_digest(archive.open('rb'),'sha256').hexdigest()}))
...
subprocess.run(['docker','build','--platform','linux/amd64','--target','runner','-t','telegram-app:multi-invoice-20260917',str(build)],check=True)
