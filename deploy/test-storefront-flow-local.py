...
base = Path('C:\\Users\\DAVIDB~1\\AppData\\Local\\Temp\\telegram-storefront-flow-local-tests-20260916-r3')
...
if '--background' in sys.argv:
    ...
    print('Isolated storefront-flow tests and backend build started'); sys.exit(0)
network = 'storefront-flow-review-isolated-20260916'
database = 'storefront-flow-review-db-20260916'
created_net = False
created_db = False
try:
    state.write_text(json.dumps({'status': 'testing'}))
    source = base / 'source'; source.mkdir(exist_ok=False)
    archive = Path('C:\\Users\\David Boy\\Documents\\NextJS\\telegram\\deploy\\storefront-flow-source.tar.gz')
    manifest = json.loads(Path('C:\\Users\\David Boy\\Documents\\NextJS\\telegram\\deploy\\storefront-flow-release-manifest.json').read_text())
    ...
    run(['docker', 'build', '--target', 'deps', '-t', 'telegram-storefront-flow-deps:20260916', str(source)])
    (source / 'Dockerfile.storefront-flow-test').write_text('FROM telegram-storefront-flow-deps:20260916\nCOPY . .\nRUN npx --no-install prisma generate\nCMD ["sh", "-c", "npx --no-install prisma migrate deploy && npx --no-install vitest run tests/storefront-web-order-db.test.ts tests/storefront-product-guidance.test.ts tests/web-success-announcement.test.ts tests/web-success-worker.test.ts tests/completion-caption-worker.test.ts tests/product-post-delivery.test.ts tests/telegram-completion-ux.test.ts tests/success-channel.test.ts"]\n')
    run(['docker', 'build', '-f', str(source / 'Dockerfile.storefront-flow-test'), '-t', 'telegram-storefront-flow-tests:20260916', str(source)])
    run(['docker', 'network', 'create', '--internal', network], stdout=subprocess.DEVNULL); created_net = True
    run(['docker', 'run', '-d', '--name', database, '--network', network, '--memory', '512m', '--tmpfs', '/var/lib/postgresql/data:rw,size=256m', '-e', 'POSTGRES_USER=storefront-flowtest', '-e', 'POSTGRES_PASSWORD=disposable-only', '-e', 'POSTGRES_DB=storefront-flow_disposable', 'postgres:17-alpine'], stdout=subprocess.DEVNULL); created_db = True
    for attempt in range(30):
        if subprocess.run(['docker', 'exec', database, 'pg_isready', '-U', 'storefront-flowtest'], capture_output=True).returncode == 0: break
        time.sleep(1)
    else: raise RuntimeError('Disposable DB not ready')
    url = 'postgresql://storefront-flowtest:disposable-only@' + database + ':5432/storefront-flow_disposable'
    ...
    run(args + ['telegram-storefront-flow-tests:20260916'], env=env)
