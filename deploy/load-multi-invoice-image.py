...
archive=base/'multi-invoice-image.tar.gz'
manifest=json.loads((base/'multi-invoice-image.json').read_text())
...
image=json.loads(subprocess.check_output(['docker','image','inspect','telegram-app:multi-invoice-20260917']))[0]
assert image['Architecture']=='amd64' and image['Os']=='linux'
subprocess.run(['python3',str(base/'verify-multi-invoice-content.py')],check=True)
