...
out=Path('deploy/storefront-flow-image.tar.gz')
with gzip.open(out,'wb',compresslevel=1) as f:
 p=subprocess.Popen(['docker','save','telegram-app:storefront-flow-20260916'],stdout=subprocess.PIPE)
...
Path('deploy/storefront-flow-image.json').write_text(json.dumps(manifest))
