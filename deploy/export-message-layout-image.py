...
out=Path('deploy/message-layout-image.tar.gz')
with gzip.open(out,'wb',compresslevel=1) as f:
 p=subprocess.Popen(['docker','save','telegram-app:message-layout-20260916'],stdout=subprocess.PIPE)
...
Path('deploy/message-layout-image.json').write_text(json.dumps(manifest))
