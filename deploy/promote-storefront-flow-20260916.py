...
candidate=('telegram-app' if kind=='backend' else 'telegram-storefront')+':storefront-flow-20260916'
...
backup=base/('backups/storefront-flow-20260916-'+str(int(time.time())));backup.mkdir(mode=0o700,exist_ok=False)
...
subprocess.run(['docker','tag',old,alias.split(':')[0]+':rollback-storefront-flow-20260916'],check=True)
db=None
if kind=='backend':
