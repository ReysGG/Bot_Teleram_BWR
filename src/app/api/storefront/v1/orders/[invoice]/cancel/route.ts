...
export async function POST(request: NextRequest, { params }: { params: Promise<{ invoice: string }> }) {
  ...
  try {
    ...
    const order = await cancelWebOrder({ customerId: session.webCustomerId, invoiceNumber: invoice });
    return NextResponse.json({ ok: true, order }, { headers: { "cache-control": "private, no-store" } });
  } catch (error) {
    return NextResponse.json({ ok: false, code: error instanceof WebCustomerAccessError ? "session_invalid" : "order_cannot_cancel" }, { status: error instanceof WebCustomerAccessError ? 401 : 409 });
  }
}
